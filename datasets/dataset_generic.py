...
class Generic_MIL_Dataset(Generic_WSI_Classification_Dataset):
	...
	def __getitem__(self, idx):
		...
		if not self.use_h5:
			if self.data_dir:
				full_path = os.path.join(data_dir, '1024_feature', '{}.pt'.format(slide_id))
...
if __name__ == '__main__':
	data_root_dir = 'my_data'

	seed = 1
	dataset = Generic_MIL_Dataset(csv_path='all_data.csv',
								  data_dir=os.path.join(data_root_dir, 'class_feature'),
								  shuffle=False,
								  seed=seed,
								  print_info=True,
								  ...
								  patient_strat=False,
								  ignore=[])
	split_gen = dataset.create_splits()
	print(split_gen)
